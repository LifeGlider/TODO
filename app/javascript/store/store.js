...
export const store = new Vuex.Store({
  ...
  actions: {
    ...
    SAVE_TASK: async (context, payload) => {
      let {data} = await axios.post('http://localhost:3000/tasks/',
      { important: payload.important, todo: payload.todo });
      context.commit('ADD_TASK', data);
    },

    UPDATE_TASK: async (context, payload) => {
      let {data} = await axios.patch(`http://localhost:3000/tasks/${payload.id}`, 
    ...
    DELETE_TASK: async (context, payload) => {
      let {data} = await axios.delete(`http://localhost:3000/tasks/${payload.id}`)
